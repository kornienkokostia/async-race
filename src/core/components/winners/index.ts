...
export default class Winners extends Component {
  ...
  createTableItem(
    ...
    const winnersTableItemWrapper = this.elFactory('div', {
      class: 'winners-table-item-wrapper',
    });
    const winnersTableItem = this.elFactory('div', {
      class: 'winners-table-item',
    });
    const winnersTableItemNumber = this.elFactory('p', {
      class: 'winners-table-item-number',
    });
    winnersTableItemNumber.textContent = `${num}`;
    const winnersTableItemCar = this.elFactory('div', {
      class: 'winners-table-item-car',
    });
    ...
    const winnersTableItemName = this.elFactory('p', {
      class: 'winners-table-item-name',
    });
    winnersTableItemName.textContent = name;
    const winnersTableItemWinns = this.elFactory('p', {
      class: 'winners-table-item-winns',
    });
    winnersTableItemWinns.textContent = `${wins}`;
    const winnersTableItemTime = this.elFactory('p', {
      class: 'winners-table-item-time',
    });
    winnersTableItemTime.textContent = `${time}`;

    winnersTableItem.append(
      winnersTableItemCar,
      winnersTableItemName,
      winnersTableItemWinns,
  ...
  async updateTable() {
    ...
    cars.map((el, i) => {
      document
        .querySelector('.winners-table-body')
        ?.append(
          this.createTableItem(
            10 * (AppState.winnersPageNum - 1) + i + 1,
            el.color,
            el.name,
            data[i].wins,
            data[i].time,
    ...
    if (
      AppState.numOfWinners < 10 ||
      AppState.numOfWinners <= AppState.winnersPageNum * 10
    ) {
  ...
  createWinnersTable() {
    ...
    const winnersTableHeader = this.elFactory('div', {
      class: 'winners-table-header',
    });

    const winnersTableHeaderCar = this.elFactory('div', {
      class: 'winners-table-header-car',
    });
    ...
    const winnersTableHeaderName = this.elFactory('div', {
      class: 'winners-table-header-name',
    });
    ...
    const createTableHeaderItem = (name: string, title: string) => {
      ...
      winnersTableHeaderItem.addEventListener('click', () => {
        if (AppState.winnersTableSortBy !== name) {
          ...
          Array.from(document.querySelectorAll('.sort-asc')).map(el =>
            el.classList.remove('active'),
          );
          Array.from(document.querySelectorAll('.sort-desc')).map(el =>
            el.classList.remove('active'),
          );
        }
    ...
    winnersTableHeader.append(
      winnersTableHeaderCar,
      winnersTableHeaderName,
      winnersTableHeaderWins,
      winnersTableHeaderTime,
    );

    const winnersTableBody = this.elFactory('div', {
      class: 'winners-table-body',
    });
    ...
    const prevBtn = this.elFactory('button', {
      class: 'winners-prev-btn btn',
      disabled: '',
    });
    prevBtn.textContent = 'Prev';
    const nextBtn = this.elFactory('button', {
      class: 'winners-next-btn btn',
      disabled: '',
    });
    ...
    nextBtn.addEventListener('click', () => {
      ...
      document.querySelector(
        '.winners-page-count',
      )!.textContent = `${AppState.winnersPageNum}`;
    });
  ...
  render() {
    this.container.append(
      this.createWinnersTitle(),
      this.createWinnersPageCount(),

...
export default class Winners extends Component {
  ...
  async updateWinnersTitle() {
    const data = await DB.getAllWinners();
    AppState.numOfWinners = data.length;
    document.querySelector('.winners-count')!.textContent = `${data.length}`;
    
  }

  createWinnersTitle(){
    const winnersTitle = this.elFactory('p', { class: 'winners-title' });
    winnersTitle.innerHTML = `Winners (<span class="winners-count">${AppState.numOfWinners}</span>)`;
    this.updateWinnersTitle()
    return winnersTitle
  }

  createWinnersPageCount(){
    const winnersPageNum = this.elFactory('p', {
      class: 'winners-page-num',
    });
    winnersPageNum.innerHTML = `Page <span class="winners-page-count">${AppState.winnersPageNum}</span>`;
    return winnersPageNum
  }

  createTableItem(num: number, color: string, name: string, wins: number, time: number){
    const winnersTableItemWrapper = this.elFactory('div', {class: 'winners-table-item-wrapper'})
    const winnersTableItem = this.elFactory('div', {class: 'winners-table-item'})
    const winnersTableItemNumber = this.elFactory('p', {class: 'winners-table-item-number'})
    winnersTableItemNumber.textContent = `${num}`
    const winnersTableItemCar = this.elFactory('div', {class: 'winners-table-item-car'})
    winnersTableItemCar.innerHTML = `
    <svg viewBox="0 0 62 19" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    const winnersTableItemName = this.elFactory('p', {class: 'winners-table-item-name'})
    winnersTableItemName.textContent = name
    const winnersTableItemWinns = this.elFactory('p', {class: 'winners-table-item-winns'})
    winnersTableItemWinns.textContent = `${wins}`
    const winnersTableItemTime = this.elFactory('p', {class: 'winners-table-item-time'})
    winnersTableItemTime.textContent = `${time}`

    winnersTableItem.append(winnersTableItemCar, winnersTableItemName, winnersTableItemWinns,
      winnersTableItemTime)

    winnersTableItemWrapper.append(winnersTableItemNumber, winnersTableItem)  

    return winnersTableItemWrapper  
  }

  async updateTable(){
    let options = {page: AppState.winnersPageNum, sortBy: '', sortOrder: ''}
    if (AppState.winnersTableSortBy !== '') {
      options.sortBy = AppState.winnersTableSortBy
      options.sortOrder = AppState.winnersTableSortOrder
    }
    const data = await DB.getAllWinners(options);
    document.querySelector('.winners-table-body')!.innerHTML = ''

    const btns = document.querySelector('.winners-pagination-btns') as HTMLDivElement
    btns.style.opacity = '0';
    
    const cars = await Promise.all(data.map(async el => {
      return await DB.getCar(el.id) as Car
    }))
    cars.map((el, i) => {
      document.querySelector('.winners-table-body')?.append(
        this.createTableItem(10 * (AppState.winnersPageNum - 1) + i + 1, el.color, el.name,
          data[i].wins, data[i].time))
    })
    if (AppState.numOfWinners > 10 * AppState.winnersPageNum) {
      document.querySelector('.winners-next-btn')!.removeAttribute('disabled')
    }
    if (AppState.winnersPageNum !== 1) {
      document.querySelector('.winners-prev-btn')!.removeAttribute('disabled')
    } else {
      document.querySelector('.winners-prev-btn')!.setAttribute('disabled', '')
    }
    if (AppState.numOfWinners < 10 || AppState.numOfWinners <= AppState.winnersPageNum * 10) {
      document.querySelector('.winners-next-btn')!.setAttribute('disabled', '')
    }
    btns.style.opacity = '1';
  }

  createWinnersTable(){
    const winnersTable = this.elFactory('div', {class: 'winners-table'})
    const winnersTableHeader = this.elFactory('div', {class: 'winners-table-header'})
    
    const winnersTableHeaderCar = this.elFactory('div', {class: 'winners-table-header-car'})
    const winnersTableHeaderCarSpan = this.elFactory('span', {})
    winnersTableHeaderCarSpan.textContent = 'Car'
    winnersTableHeaderCar.append(winnersTableHeaderCarSpan)

    const winnersTableHeaderName = this.elFactory('div', {class: 'winners-table-header-name'})
    const winnersTableHeaderNameSpan = this.elFactory('span', {})
    winnersTableHeaderNameSpan.textContent = 'Name'
    winnersTableHeaderName.append(winnersTableHeaderNameSpan)

    const createSortingBtnAsc = () => this.elFactory('img', {class: 'sort-asc', 
      src: './assets/images/icons/sort-asc.svg', 
      alt: 'sort-asc'})

    const createSortingBtnDesc = () => this.elFactory('img', {class: 'sort-desc', 
      src: './assets/images/icons/sort-desc.svg', 
      alt: 'sort-desc'})

    const createTableHeaderItem = (name: string, title: string) => {
      const winnersTableHeaderItem = this.elFactory('div', {class: `winners-table-header-${name}`})
      const winnersTableHeaderItemSpan = this.elFactory('span', {})
      winnersTableHeaderItemSpan.textContent = title
      const winnersTableHeaderItemSort = this.elFactory('div', {class: 'winners-table-header-sort'})
      const winnersTableHeaderItemSortAsc = createSortingBtnAsc()
      const winnersTableHeaderItemSortDesc = createSortingBtnDesc()
      winnersTableHeaderItemSort.append(winnersTableHeaderItemSortAsc, winnersTableHeaderItemSortDesc)
      winnersTableHeaderItemSortAsc.ondragstart = () => false;
      winnersTableHeaderItemSortDesc.ondragstart = () => false;
      
      winnersTableHeaderItem.append(winnersTableHeaderItemSpan, winnersTableHeaderItemSort)

      if (AppState.winnersTableSortBy === name) {
        if (AppState.winnersTableSortOrder === 'desc') {
          winnersTableHeaderItemSortAsc.classList.remove('active')
          winnersTableHeaderItemSortDesc.classList.add('active')
        } else {
          winnersTableHeaderItemSortAsc.classList.add('active')
          winnersTableHeaderItemSortDesc.classList.remove('active')
        }
      }

      winnersTableHeaderItem.addEventListener('click', () => {
        if (AppState.winnersTableSortBy !== name) {
          AppState.winnersTableSortOrder = 'desc'
          Array.from(document.querySelectorAll('.sort-asc')).map(el => el.classList.remove('active'))
          Array.from(document.querySelectorAll('.sort-desc')).map(el => el.classList.remove('active'))
        }
        AppState.winnersTableSortBy = name
       
        if (AppState.winnersTableSortOrder === 'desc') {
          AppState.winnersTableSortOrder = 'asc'
          winnersTableHeaderItemSortAsc.classList.add('active')
          winnersTableHeaderItemSortDesc.classList.remove('active')
        } else {
          AppState.winnersTableSortOrder = 'desc'
      ...
      return winnersTableHeaderItem
    }  

    const winnersTableHeaderWins = createTableHeaderItem('wins', 'Wins')
    const winnersTableHeaderTime = createTableHeaderItem('time', 'Best time')

    winnersTableHeader.append(winnersTableHeaderCar, winnersTableHeaderName, winnersTableHeaderWins, winnersTableHeaderTime)

    const winnersTableBody = this.elFactory('div', {class: 'winners-table-body'})

    this.updateTable()

    const paginationBtns = this.elFactory('div', {
      class: 'winners-pagination-btns',
    });
    const prevBtn = this.elFactory('button', {
      class: 'winners-prev-btn btn', disabled: ''
    });
    prevBtn.textContent = 'Prev';
    const nextBtn = this.elFactory('button', {
      class: 'winners-next-btn btn', disabled: ''
    });
    nextBtn.textContent = 'Next';

    nextBtn.addEventListener('click', () => {
      AppState.winnersPageNum ++
      this.updateTable()
      document.querySelector('.winners-page-count')!.textContent = `${AppState.winnersPageNum}`
    })
    ...
    winnersTable.append(winnersTableHeader, winnersTableBody, paginationBtns)
  ...
  render() {
    this.container.append(this.createWinnersTitle(), this.createWinnersPageCount(), this.createWinnersTable())

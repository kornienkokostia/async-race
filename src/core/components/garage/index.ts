...
export default class Garage extends Component {
  ...
  private winner = '';
  private carsStopped = true 
  private allRaceCars = 0
  ...
  createGarageHeader() {
    ...
    const garageCreateInput = this.elFactory('input', {
      class: 'garage-create-input input-text',
      type: 'text',
      placeholder: 'Car name',
      value: AppState.createInputText,
    });

    const garageCreateColorPicker = this.elFactory('input', {
      class: 'garage-create-color input-color',
      type: 'color',
      value: AppState.createInputColor,
    });
    const garageCreateColorPickerWrapper = this.elFactory(
      'div',
      { class: 'garage-create-color-wrapper input-color-wrapper' },
      garageCreateColorPicker,
    );

    const createBtn = this.elFactory('button', {
      class: 'garage-create-btn btn',
    });
    ...
    createBtn.addEventListener('click', () => {
      if (garageCreateInput.value !== '') {
        DB.addCar({
          name: garageCreateInput.value,
          color: garageCreateColorPicker.value,
        });
    ...
    garageCreate.append(
      garageCreateInput,
      garageCreateColorPickerWrapper,
      createBtn,
    );

    const garageUpdate = this.elFactory('div', { class: 'garage-update' });

    const garageUpdateInput = this.elFactory('input', {
      class: 'garage-update-input input-text',
      type: 'text',
      placeholder: 'New car name',
      value: AppState.updateInputText,
    });

    const garageUpdateColorPicker = this.elFactory('input', {
      class: 'garage-update-color input-color',
      type: 'color',
      value: AppState.updateInputColor,
    });
    const garageUpdateColorPickerWrapper = this.elFactory(
      'div',
      { class: 'garage-update-color-wrapper input-color-wrapper' },
      garageUpdateColorPicker,
    );

    const updateBtn = this.elFactory('button', {
      class: 'garage-update-btn btn',
    });
        ...
        DB.updateCar(
          {
            ...
            color: garageUpdateColorPicker.value,
          },
          +carId,
        );
        this.updateGarageAllCars();
    ...
    garageUpdate.append(
      garageUpdateInput,
      garageUpdateColorPickerWrapper,
      updateBtn,
    );

    const headerButtons = this.elFactory('div', {
      class: 'garage-header-btns',
    });

    const raceBtnImg = this.elFactory('img', {
      class: 'header-btn-img',
      src: './assets/images/icons/race-btn.svg',
      alt: 'header-btn-img',
    });
    const raceBtnSpan = this.elFactory('span', {});
    raceBtnSpan.textContent = 'Race';
    const raceBtn = this.elFactory(
      'button',
      { class: 'header-btn garage-race-btn btn' },
      raceBtnImg,
      raceBtnSpan,
    );
    ...
    raceBtn.addEventListener('click', () => {
      ...
      cars.map(el => {
        ...
        if (!carStateP.classList.contains('active')) {
          ...
          this.stopCarEngine(+carId, carModel);
        }
    ...
    const resetBtnImg = this.elFactory('img', {
      class: 'header-btn-img',
      src: './assets/images/icons/reset-btn.svg',
      alt: 'header-btn-img',
    });
    const resetBtnSpan = this.elFactory('span', {});
    resetBtnSpan.textContent = 'Reset';
    const resetBtn = this.elFactory(
      'button',
      { class: 'header-btn garage-reset-btn btn', disabled: '' },
      resetBtnImg,
      resetBtnSpan,
    );
    ...
    resetBtn.addEventListener('click', () => {
      this.carsStopped = true
      this.winner = '';
      document.querySelector('.garage-winner-message span')!.textContent = '';
    ...
    const generateCarsBtn = this.elFactory('button', {
      class: 'garage-generate-cars-btn btn',
    });
    ...
    generateCarsBtn.addEventListener('click', () => {
      for (let i = 0; i < 100; i++) {
        ...
        const carModel =
          cars[randomNumName].models[
            randomNum(0, cars[randomNumName].models.length)
          ];
  ...
  async startCarEngine(id: number, car: HTMLDivElement, carName: string) {
    ...
    const time = +carInfo!.distance / +carInfo!.velocity / 1000;
    const roundedTime = +(Math.round(time * 100) / 100).toFixed(2);
    car.style.animation = `animate ${time}s linear 0s 1 
    ...
    if (driveMode === 'error') {
      ...
      if (this.winner === '' && !this.carsStopped) {
        this.winner = carName;
        const winnerInDB = (await DB.getWinner(id)) as Winner;
        if (Object.keys(winnerInDB).length === 0) {
          await DB.addWinner({ id: id, wins: 1, time: roundedTime });
        } else {
          const newTime =
            winnerInDB.time > roundedTime ? roundedTime : winnerInDB.time;
          await DB.updateWinner(
            { wins: (winnerInDB.wins += 1), time: newTime },
            id,
          );
        }
        document.querySelector(
          '.garage-winner-message span',
        )!.textContent = `${carName} went first (${roundedTime}s)`;
      }
      
    }
    this.allRaceCars++
    if (this.allRaceCars === document.querySelector('.garage-cars')?.children.length) {
      document.querySelector('.garage-reset-btn')?.removeAttribute('disabled');
      this.allRaceCars = 0
    }
  }

  async stopCarEngine(id: number, carModel: HTMLDivElement) {
  ...
  createCar(name: string, color: string, id: number) {
    const garageCar = this.elFactory('div', {
      class: 'garage-car',
      'car-id': `${id}`,
    });
    const garageCarHeader = this.elFactory('div', {
      class: 'garage-car-header',
    });

    const garageCarName = this.elFactory('p', { class: 'garage-car-name' });
    garageCarName.textContent = name;

    const garageCarEditImg = this.elFactory('img', {
      class: 'garage-car-header-btn-img',
      src: './assets/images/icons/edit-car-btn.svg',
      alt: 'garage-car-edit-btn-img',
    });
    const garageCarEditBtn = this.elFactory(
      'button',
      { class: 'garage-car-edit-btn garage-car-header-btn' },
      garageCarEditImg,
    );
    ...
    garageCarEditBtn.addEventListener('click', () => {
      ...
      const editColor = document.querySelector(
        '.garage-update-color',
      ) as HTMLInputElement;
    ...
    const garageCarDeleteImg = this.elFactory('img', {
      class: 'garage-car-header-btn-img',
      src: './assets/images/icons/delete-car-btn.svg',
      alt: 'garage-car-delete-btn-img',
    });
    const garageCarDeleteBtn = this.elFactory(
      'button',
      { class: 'garage-car-delete-btn garage-car-header-btn' },
      garageCarDeleteImg,
    );
    ...
    garageCarDeleteBtn.addEventListener('click', async () => {
      ...
      if (
        ...
        AppState.carsPageNum--;
      }
      this.updateGarageAllCars();
      this.updateGarageTitle();
      document.querySelector(
        '.cars-page-count',
      )!.textContent = `${AppState.carsPageNum}`;
      AppState.numOfCars--;
    ...
    const garageCarState = this.elFactory('div', {
      class: 'garage-car-state-wrapper',
    });
    const garageCarStateP = this.elFactory('span', {
      class: 'garage-car-state car-state-p active',
    });
    ...
    const garageCarStateD = this.elFactory('span', {
      class: 'garage-car-state car-state-d',
    });
    garageCarStateD.textContent = 'D';
    const garageCarStateCover = this.elFactory('div', {
      class: 'garage-car-state-cover',
    });
    ...
    const garageCarFinish = this.elFactory('img', {
      class: 'garage-car-finish',
      src: './assets/images/icons/finish.svg',
      alt: 'garage-car-finish',
    });
  ...
  async updateGarageAllCars() {
    const data = await DB.getAllCarsWithinPage(AppState.carsPageNum);
    ...
    if (AppState.carsPageNum === 1) {
      document
        .querySelector('.garage-cars-prev-btn')!
    ...
    if (AppState.numOfCars < 7 || AppState.numOfCars <= AppState.carsPageNum * 7) {
      document
        .querySelector('.garage-cars-next-btn')!
        .setAttribute('disabled', '');
    } else {
      document
        .querySelector('.garage-cars-next-btn')!
  ...
  createGarageCars() {
    const garageCarsWrapper = this.elFactory('div', {
      class: 'garage-cars-wrapper',
    });
    ...
    const garageCarsPageNum = this.elFactory('p', {
      class: 'garage-cars-page-num',
    });
    garageCarsPageNum.innerHTML = `Page <span class="cars-page-count">${AppState.carsPageNum}</span>`;
    ...
    const paginationBtns = this.elFactory('div', {
      class: 'garage-cars-pagination-btns',
    });
    const prevBtn = this.elFactory('button', {
      class: 'garage-cars-prev-btn btn',
    });
    prevBtn.textContent = 'Prev';
    const nextBtn = this.elFactory('button', {
      class: 'garage-cars-next-btn btn',
    });
  ...
  createWinnerMessage() {
    const winnerMsg = this.elFactory('div', { class: 'garage-winner-message' });
    const winnerMsgText = this.elFactory('span', {});
    winnerMsg.append(winnerMsgText);
    return winnerMsg;
  }
